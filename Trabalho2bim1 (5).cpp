...
int menu_relatorios(){
	...
	yellow();
	gotoxy(32, 6);
	printf("[A]-Autores que começam com uma determinada letra, ordenados por nacionalidade");
	gotoxy(32, 7);
	printf("[B]-Livros com Títulos que contenham uma determinada palavra");
	gotoxy(32, 8);
	printf("[C]- Empréstimos de uma pessoa específica");
	gotoxy(32, 9);
	printf("[D]- Dados de livros de um autor específico");
	gotoxy(32, 10);
	printf("[E]-Empréstimos separados por pessoa");
	printf("[ESC]-Voltar");
	gotoxy(32, 15);
	printf("Opcao Desejada:");
	...
	if(PtrLv == NULL){
		...
		while(op=='S' && verifica_caracter(ver_caracter) == 0){
			...
			if(pos==-1){
				...
				printf("Ano de Publicacao: %d",lv.ano_pub);
				gotoxy(4,i++);
				printf("Deseja alterar o Tudo(S/N)?");
				op = toupper(getch());
				system("cls");
				MolduraCRUD();
				if(op=='S'){
	...
	if(PtrPes == NULL) {
		...
		{	
			if(pes.status){
				...
				i++;
				gotoxy(4,i);
				printf("Endereço: %s",pes.endereco);
				i+=2;
				gotoxy(4,i);
			}			
			fread(&pes,sizeof(TPpessoas),1,PtrPes);
...
int verifica_termo(char aux_titulo[], char palavra[]) {
    int i=0,j=0,x=0,nPalavras=0,tam,igual=0;
    char titulo[30];
    
 //   for (int a=0;aux_titulo[a]!='\0';a++){
//    	titulo[a]==tolower(aux_titulo[a]);
//    }
//    titulo[a]='\0';
    printf("%s",titulo);
    getch();
